-- Buklat — full schema for a fresh Supabase project.
...
,
update,
delete on public.chapters to authenticated;
...
,
update,
delete on public.reading_progress to authenticated;
...
create policy stories_public_read on public.stories for
select using (status = 'published');

drop policy if exists stories_author_read on public.stories;

create policy stories_author_read on public.stories for
select to authenticated using (auth.uid () = author_id);

drop policy if exists stories_author_insert on public.stories;

create policy stories_author_insert on public.stories for
insert
    to authenticated
with
...
create policy stories_author_update on public.stories for
...
create policy chapters_public_read on public.chapters for
select using (
        exists (
            select 1
            from public.stories s
            where
                s.id = chapters.story_id
...
create policy chapters_author_read on public.chapters for
...
create policy chapters_author_write on public.chapters for
insert
    to authenticated
with
    check (
...
create policy chapters_author_update on public.chapters for
...
create policy progress_own on public.reading_progress for all to authenticated using (auth.uid () = user_id)
with
    check (auth.uid () = user_id);

drop policy if exists likes_public_read on public.story_likes;

create policy likes_public_read on public.story_likes for
select using (true);

drop policy if exists likes_insert_own on public.story_likes;

create policy likes_insert_own on public.story_likes for
insert
    to authenticated
with
    check (auth.uid () = user_id);
...
drop policy if exists Buklat_media_read on storage.objects;

create policy Buklat_media_read on storage.objects for
select using (
        bucket_id in ('covers', 'avatars')
    );

drop policy if exists Buklat_media_insert on storage.objects;

create policy Buklat_media_insert on storage.objects for insert to authenticated
  with check (bucket_id in ('covers', 'avatars') and (storage.foldername(name))[1] = auth.uid()::text);

drop policy if exists Buklat_media_update on storage.objects;

create policy Buklat_media_update on storage.objects for update to authenticated
  using (bucket_id in ('covers', 'avatars') and (storage.foldername(name))[1] = auth.uid()::text);

drop policy if exists Buklat_media_delete on storage.objects;

create policy Buklat_media_delete on storage.objects for delete to authenticated
  using (bucket_id in ('covers', 'avatars') and (storage.foldername(name))[1] = auth.uid()::text);